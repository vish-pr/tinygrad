...
def merge_dicts(ds: Iterable[Dict[T, U]]) -> Dict[T, U]:
  assert len(kvs := set([(k, v) for d in ds for k, v in d.items()])) == len(set(kv[0]
                                                                                for kv in kvs)), f"cannot merge, {kvs} contains different values for the same key"
...
class dtypes:
  ...
  def is_int(x: DType) -> bool: return x in (dtypes.int8, dtypes.int16, dtypes.int32,
                                             dtypes.int64, dtypes.uint8, dtypes.uint16, dtypes.uint32, dtypes.uint64)
...
class GlobalCounters:
  ...
  allocs_done: ClassVar[List[Tuple[bool, int]]] = []
  max_mem_used: ClassVar[int] = 0
  @staticmethod
  def reset(): GlobalCounters.global_ops, GlobalCounters.global_mem, GlobalCounters.time_sum_s, GlobalCounters.kernel_count, GlobalCounters.allocs_done, GlobalCounters.max_mem_used = 0, 0, 0.0, 0, [], 0
...
def diskcache_put(table: str, key: Union[Dict, str, int], val: Any):
  ...
  cur.execute(
    f"REPLACE INTO {table}_{VERSION} ({', '.join(key.keys())}, val) VALUES ({', '.join(['?']*len(key.keys()))}, ?)", tuple(key.values()) + (pickle.dumps(val), ))
  conn.commit()
  cur.close()
  return val
